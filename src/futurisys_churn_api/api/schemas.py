...
class EmployeeData(BaseModel):
    ...
    revenu_mensuel: int = Field(..., example=5000)
    ...
    annes_sous_responsable_actuel: int = Field(..., example=2)
    ...
    augementation_salaire_precedente: int = Field(..., example=15)
    nombre_participation_pee: int = Field(..., example=1)
    nb_formations_suivies: int = Field(..., example=3)
    nombre_employee_sous_responsabilite: int = Field(..., example=4)
    distance_domicile_travail: int = Field(..., example=10)
    niveau_education: int = Field(..., example=4)
